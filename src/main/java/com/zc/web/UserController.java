...
public class UserController {
	...
	public String login(String check, Integer permission, String userNo, String password, Model model, HttpServletRequest request, HttpServletResponse response) {
        System.out.println(permission + "权限");
        //这个当前用户验证的是user表中的信息，currentUser是user表中查询到的，
        // 这时只有userNo，password和permission三个属性，要获取当前用户的详细信息还需要接下来的进一步查询//1.用户登录，返回查询到的用户对象
        User currentUser = IUserService.login(userNo, password);
        //控制台测试
        ...
        if (currentUser == null) {
            model.addAttribute("message", "提示：用户名或密码错误！");
            return "../../index.jsp";
        }
        if (permission == null||"".equals(permission)) {
            model.addAttribute("message", "提示：请选择用户类型！");
            return "../../index.jsp";
        }
        //检查验证码输入
...
//            writer.println("<script>alert('登录失败了！验证码错误！！！');</script>");
            model.addAttribute("message", "提示：验证码输入错误！");
            return "../../index.jsp";
        }
	...
	public String modifyPassword(Model model,String newPassword1,String currentUserNo) {
		...
		int num = IUserService.modifyPassword(currentUserNo, newPassword1);
